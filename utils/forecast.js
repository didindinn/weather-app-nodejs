...
const forecast = (latitude, longitude, callback) => {
  ...
  request({ url: URL, json: true }, (error, response) => {
    ...
    if (error) {
      ...
      return callback(
        undefined,
        `${data.daily.data[0].summary} دمای کنونی هوا ${data.currently.temperature} درجه سانتی گراد میباشد.\بیشترین دما: ${data.daily.data[0].temperatureHigh}درجه سانتی گراد میباشد.
